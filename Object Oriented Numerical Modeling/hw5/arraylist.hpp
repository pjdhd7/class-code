...
{
    if(i >= m_size)
        throw numErr("ArrayList<T>::[]: Invalid Index");
...
template <typename T>
bool ArrayList<T>::operator==(ArrayList<T> rhs)
{
...
std::istream& operator>>(std::istream& is, ArrayList<T>& alist)
{
    ...
    if(is.good() == false || is.eof() == true)
        throw streamErr("Bad Input stream");

    //Sets up a temporary container for the input
    //Then checks for bad input
    T inputTemp;
    is >> inputTemp;

    if(is.fail() == true) throw inputErr("Malformed Input");

    //Puts the extracted data at the end of the ArrayList
    alist.insert_back(inputTemp);
